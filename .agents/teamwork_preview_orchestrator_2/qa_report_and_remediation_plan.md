...
**Date**: 2026-09-05  
**Orchestrator**: Project Orchestrator (Successor: `teamwork_preview_orchestrator_2`)  
...
2. [Deep Dive: Showcase Missing Videos & Mobile Video Handling](#2-deep-dive-showcase-missing-videos--mobile-video-handling)
...
A comprehensive, read-only quality assurance scan was conducted across the `webinar` and `payment` pages of the Raise AI project prior to the launch of paid ad campaigns. Specialized exploration and adversarial review agents thoroughly audited the HTML structure, CSS layouts, JavaScript logic, media assets on disk, conversion tracking pixels, and end-to-end user flows.

### Critical Takeaways:
1. **Showcase Videos #4 and #6 Return HTTP 404**: Both videos fail to load because of an exact character encoding mismatch. `webinar/index.html` references ASCII period `.` in lines 645 (`discus._`) and 675 (`comme._`), whereas the physical files on disk in `webinar/Webinar videos/` use the Unicode Horizontal Ellipsis glyph `…` (`U+2026`).
2. **Showcase Lacks Posters & Relies on Fragile Thumbnail Seek**: None of the 6 showcase videos define a `poster="..."` attribute. The inline script (`webinar/index.html` lines 711–715) attempts to seek `currentTime = 2.0` on `loadedmetadata` to grab a thumbnail frame. On mobile devices (iOS Safari, Android Chrome power-saver mode), this seek does not render without user interaction, causing the video cards to appear as blank black rectangles.
3. **Severe Conversion Tracking Flaw (False Purchases)**: In `webinar/payment/index.html`, standard Meta Pixel `Purchase` events (₹99 revenue) are triggered prematurely in two separate functions: `trackGPayClick()` (lines 173–187) and `trackPurchaseEvent()` (lines 441–450). These fire whenever a visitor clicks utility buttons ("Copy UPI ID", "Download QR", opening guides, or tapping WhatsApp). Meta's algorithm will optimize for users who click buttons rather than actual paying customers, inflating ROAS by 10x–20x and burning ad spend.
...
## 2. Deep Dive: Showcase Missing Videos & Mobile Video Handling
...
1. **Character Encoding Typo**:
   - For Card #4, the HTML URL contains `discus._` using the standard ASCII dot character (`.` / `U+002E`). The file on disk was saved from an automated generator containing the Unicode Horizontal Ellipsis character `…` (`U+2026` / UTF-8 byte sequence `0xE2 0x80 0xA6`): `Character_wearing_clothes_discus…_202608302136.mp4`.
...
### 2.2 Showcase Script & Mobile Video Playback Analysis
- **File**: `c:\Users\hadhi\OneDrive\Desktop\Raise AI\webinar\index.html`, Lines 689–748
- **Code Analysis**:
  - The script block nesting is syntactically balanced (lines 745–747 accurately close `overlay.addEventListener`, `overlays.forEach`, and `DOMContentLoaded`, followed by `</script>` at line 748).
  - However, the script relies on programmatic thumbnail seeking at lines 711–715:
    ```javascript
    video.addEventListener('loadedmetadata', () => {
        video.currentTime = 2.0;
    });
    ```
  - **Issue**: On mobile Safari (iOS WebKit) and modern Android Chrome, seeking video without a preceding user gesture is frequently throttled or ignored to preserve cellular data. Without static `poster` images, the video cards render as blank black rectangular blocks until tapped.
  - **Remediation**: Provide static poster image files (or generated WebP frames) via the `poster="..."` attribute on each `<video>` element, and add robust playsinline configuration for smooth in-carousel playback.
...
- **Lines**: 173–187 (`trackGPayClick`) and 441–450 (`trackPurchaseEvent`)
- **Direct Code Observation**:
  1. `trackGPayClick()` at lines 173–187:
     ```javascript
     ...
     ```
  2. `trackPurchaseEvent()` at lines 441–450:
     ```javascript
...
| **Showcase 404 Videos & Missing Posters** | 🔴 Critical | Video portfolio appears broken; user cannot evaluate AI video quality; instant credibility loss. |
...
This step-by-step plan outlines the exact code modifications required. All changes are concrete, specific, and ready for execution upon approval.
...
### Step 2: Add Video Posters & Harden Showcase Carousel Script
**Goal**: Add explicit `poster="..."` images to each `<video>` element in `webinar/index.html` so mobile devices don't display blank black frames, and ensure smooth in-line mobile playback.

1. **In `webinar/index.html`**, update `<video>` elements (lines 598, 613, 628, 643, 658, 673) to include `playsinline` and explicit `poster`:
   ```html
   <!-- Example for Card #1 -->
   <video class="w-full h-full object-cover" preload="metadata" playsinline muted poster="/webinar/Webinar%20videos/thumbnails/thumb_1.jpg">
       <source src="/webinar/Webinar%20videos/Video%20Project%202%20(1).mp4" type="video/mp4">
   </video>
   ```
2. **In `webinar/index.html`**, harden the click listener in lines 717–745 to ensure the overlay properly hides and the video plays unmuted upon tap:
   ```javascript
...
**Goal**: Stop firing standard `Purchase` events on clicks in `webinar/payment/index.html`. Replace with custom non-revenue tracking events, and record true `Purchase` only on `thankyou/index.html` using the project's actual Pixel ID (`2034324397452606`).

1. **In `webinar/payment/index.html`**, replace `trackGPayClick` (lines 173–187):
   ```javascript
...
2. **In `webinar/payment/index.html`**, replace `trackPurchaseEvent` (lines 441–450):
   ```javascript
...
4. **Add True Purchase Event on `thankyou/index.html`**:
   Add the Meta Pixel snippet using the project's actual Pixel ID (`2034324397452606`) inside `<head>` of `thankyou/index.html`:
   ```html
   <script>
...
| **Showcase Carousel** | Click `#showcase-prev`, `#showcase-next`, and video overlays in browser | Carousel slides smoothly; video plays with audio on tap | [ ] |
...
| **Thank You Page Pixel** | Visit `raiseai.in/thankyou/` | Fires `PageView` and true `Purchase` event (₹99) on Pixel ID `2034324397452606` | [ ] |
...
---
*Report certified and delivered by Project Orchestrator (Generation 2: `teamwork_preview_orchestrator_2`).*
