...
const images = [
    'https://images.unsplash.com/photo-1604681630513-69474a4e253f?q=80&w=400&auto=format&fit=crop', // Fashion/Model
    'https://images.unsplash.com/photo-1558981403-c5f9899a28bc?q=80&w=400&auto=format&fit=crop', // Scooter/Street
    'https://images.unsplash.com/photo-1599643478514-4a4204128bd1?q=80&w=400&auto=format&fit=crop', // Jewelry
    'https://images.unsplash.com/photo-1557804506-669a67965ba0?q=80&w=400&auto=format&fit=crop', // Speaking/Professional
    'https://images.unsplash.com/photo-1550745165-9bc0b252726f?q=80&w=400&auto=format&fit=crop', // Retro/Tech/Illustration proxy
    'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=400&auto=format&fit=crop'  // Person looking at camera
];
...
images.forEach((imgUrl, i) => {
    itemsHtml += `
                    <!-- Item ${i+1} -->
                    <div class="snap-center shrink-0 w-[240px] md:w-[280px] aspect-[9/16] relative rounded-2xl overflow-hidden group/card cursor-pointer border border-[#222] hover:border-neon/50 transition-all duration-300 shadow-lg hover:shadow-[0_0_25px_rgba(163,253,0,0.15)]">
                        <img src="${imgUrl}" class="absolute inset-0 w-full h-full object-cover opacity-80 group-hover/card:opacity-100 transition-opacity duration-500" alt="Showcase ${i+1}" />
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent opacity-80 group-hover/card:opacity-60 transition-opacity"></div>
                        <div class="absolute inset-0 flex items-center justify-center">
                            <div class="w-14 h-14 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center group-hover/card:bg-neon group-hover/card:border-neon group-hover/card:scale-110 transition-all duration-300 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
...
const insertMarker = '<!-- Section Divider Quote & Workflow Infographic -->';
if (html.includes(insertMarker)) {
    html = html.replace(insertMarker, showcaseSection + '\n    ' + insertMarker);
    fs.writeFileSync('index.html', html);
    console.log('Successfully injected showcase section.');
} else {
