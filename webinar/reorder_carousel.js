...
// The new desired order: the three Video Project files first, then the others
const newVideos = [
    ...
    itemsHtml += `
                    ...
                    <div class="snap-center shrink-0 w-[240px] md:w-[280px] aspect-[9/16] relative rounded-2xl overflow-hidden group/card border border-[#222] hover:border-neon/50 transition-all duration-300 shadow-lg hover:shadow-[0_0_25px_rgba(163,253,0,0.15)] bg-black">
                        ...
                        <div class="showcase-overlay absolute inset-0 flex flex-col justify-center items-center cursor-pointer transition-opacity duration-300 z-10">
                            ...
                            <div class="relative w-14 h-14 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex flex items-center justify-center group-hover/card:bg-neon group-hover/card:border-neon group-hover/card:scale-110 transition-all duration-300 shadow-[0_0_15px_rgba(0,0,0,0.5)]">
...
// The wrapper is <div id="showcase-carousel" ...>
// We need to replace everything inside it up to the end of the items.
...
if(replaceRegex.test(html)) {
    ...
    console.log('Successfully reordered showcase videos!');
} else {
